      ...
      <div className="timings-inner">
        ...
        <div className="timings-grid reveal reveal-delay-1">
          {data.batches.map((batch, idx) => (
            <div
              key={idx}
              className={`time-card ${batch.isActive ? 'active' : ''}`}
              aria-label={`${batch.name} timing`}
            >
        ...
        <div className="timings-closed reveal reveal-delay-2" role="note">
          ...
          <span
            dangerouslySetInnerHTML={{
              __html: data.closedNote.replace(
                'Sunday:',
                '<strong style="color:var(--text-secondary);">Sunday:</strong>',
              ),
            }}
          ></span>
        </div>
